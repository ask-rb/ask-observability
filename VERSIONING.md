...
gemchain guard ask-observability
gemchain update ask-observability <new-version> --dry-run
gemchain update ask-observability <new-version>
```
...
3. Build passes — `bundle exec rake build` (or `gem build ask-observability.gemspec`).
4. Changelog — `Unreleased` entries moved under the new `X.Y.Z` heading with
   the release date.
5. Version — bumped in `lib/ask/observability/version.rb` by gemchain, not
   by hand.
...
| `lib/ask/observability/version.rb` at HEAD | `X.Y.Z` |
| Git tag | `vX.Y.Z` |
